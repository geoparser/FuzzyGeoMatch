...
public class Trie {
	 ...
	 * 
	 * Index String could choose to be space removed or not. But, All the
	 * chars are deaccented and lowered.
	 * 
	...
	/**
	 * This version added haibo's other language and Country, adjective
	 * strings.
	 * 
	 * @param filename
	 * @throws IOException
	...
	public void buildTrieFromGeoNamesForIndex(String filename) throws IOException {
		...
		String s[];
		while ((e = namesreader.readLine()) != null) {
			System.out.print((++i % 1000000 == 0) ? i + " has been loaded. 8,000,000 in total.\n" : "");
			if (i % 25000000 == 0)
				System.gc();
			s = e.split("	");
			addWord2Trie(s[1], Long.parseLong(s[0]));
			// added by Haibo
			if (s[7].equals("PCLI") || s[7].equals("PCLS") || s[7].equals("PPLC") || s[7].equals("PPLA")
					|| s[7].equals("PCLIX") || s[7].equals("ADM1") || s[7].equals("ADM2")) {
		...
		while ((line = fin.readLine()) != null) {
			...
			if (str[5].indexOf(",") == -1) {
				...
				addWord2Trie(str[5], Long.parseLong(str[18])); // add
												// adj
												// mapping
			} else
				for (String sub : str[5].split(",")) {
					...
					if (!savespace)
						sub = sub.replaceAll("[ ]", "");
					addWord2Trie(sub, Long.parseLong(str[18])); // add
													// adj
													// mapping
				}
			if (str[6].indexOf(",") == -1) {
				...
				addWord2Trie(str[6], Long.parseLong(str[18])); // add
												// citizen
												// mapping
			} else
	...
	public String search(String s) {
		...
		while (father.isChildrenEmpty() == false) {
			if (father.containsChild(chars[i])) {
				if (i == chars.length - 1)
					if (father.getChild(chars[i]).isLocation()) {
				...
				if (father.isLocation()) {
					// long[] a = father.getIDValue();
					// for(int b:a)System.out.println(b);
					// "partial match, but the matched part a location:"
					// + i;
					return "PL" + i;
				} else
					// partial match, and the matched part is not a
					// location.
					return "PN" + i;
			}
		}
	...
	public static void main(String args[]) throws IOException {
		...
		Trie trie = new Trie("GeoNames/" + "SRC_cities1000.txt", Trie.SaveSpace, Trie.NoAddOrigStr);
