...
public class GazIndexer {
	...
	/**
	 * get the unigram,bigram,trigram,and position_unigram features, and store
	 * them in the static fields of StringUtil: f_unigram f_bigram f_trigram
	 * f_positiongram
	 * 
	 * @param phrase
	 */
	...
	/**
	 * This function is different from indexGazetteer. it indexes the
	 * "otherlanguage" field, and duplicates appear. Consequently, the
	 * otherlang field is removed from the document field.
	 * 
	 * @param br
	 * @param iw
	 * @throws IOException
	...
	void indexGazatteerWithOtherLanguages(BufferedReader br, IndexWriter iw) throws IOException, InterruptedException {
		...
		Field normws = new Field("NORM-WS", false, "", Field.Store.YES, Index.NOT_ANALYZED, TermVector.NO);
		Field normnws = new Field("NORM-NO-WS", false, "", Field.Store.YES, Index.NOT_ANALYZED, TermVector.NO);
		// Field sfotherlang = new Field("OTHERLANG", false, "",
		// Field.Store.YES, Index.NOT_ANALYZED, TermVector.NO);
		Field sfunigram = new Field("UNIGRAM", false, "", Field.Store.YES, Index.ANALYZED, TermVector.NO);
		Field sfbigram = new Field("BIGRAM", false, "", Field.Store.YES, Index.ANALYZED, TermVector.NO);
		Field sftrigram = new Field("TRIGRAM", false, "", Field.Store.YES, Index.ANALYZED, TermVector.NO);
		Field sfposition = new Field("POSITION", false, "", Field.Store.YES, Index.ANALYZED, TermVector.NO);
		...
		while ((line = br.readLine()) != null) {
			...
			String[] column = line.trim().split("\t");
			String key = column[1];
			String otherlang[] = column[3].split(",");
			ArrayList<String> phrases = new ArrayList<String>(Arrays.asList(otherlang));
			phrases.add(key);

			for (String phrase : phrases) {
				long lid = Long.parseLong(column[0]);
				String latitude = column[4];
				String longtitude = column[5];
				double dlong = 999, dla = 999;
				if (latitude != null) {
					dlong = Double.parseDouble(longtitude);
					dla = Double.parseDouble(latitude);
				}
				...
				long longpop = -1;
				if (column[14] != null)
					longpop = Long.parseLong(column[14]);
				String timezone = column[17];
